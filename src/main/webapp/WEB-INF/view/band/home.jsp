...
<body>
	<div class="container-lg d-flex align-items-start">
				...
				<c:choose>
					<c:when test="${member.memberStatus eq 'accept' }">
						<button type="button" class="btn w-100 text-center" style="background-color: ${bandRoom.bandRoomColor};" data-bs-toggle="modal"
							data-bs-target="#postWriteModal">글쓰기</button>
					</c:when>
					<c:when test="${member.memberStatus eq 'request' }">
						<button type="button" class="btn w-100 text-center" style="background-color: ${bandRoom.bandRoomColor};">
							가입대기중
						</button>
					</c:when>
					<c:otherwise>
		...
		<div class="flex-grow-1 flex-column" style="min-width: 500px; background-color:aliceblue;">
			<input type="text" placeholder="글 내용,#태그,@작성자 검색" class="w-100 border border-1" style="outline:none;" />
			<div style="min-height: 100px;">여기는 글을 보여주는 곳이에요!</div>
			<div style="min-height: 100px;">하지만 아직 글이 없어요 ㅜㅜ</div>
		</div>
